...
// Supports geolocation-based auto-selection via ef-geo attribute
// ============================================================================
...
class FormValidator {
  constructor() {
    ...
    this.geoData = null; // Stores fetched geolocation data
  ...
  injectLumosOverrides() {
    ...
    style.textContent = `
      ...
      /* --- DISABLED PHONE INPUT STATE --- */
      [ef-form-div] input[ef-form-input="phone"].ef-phone-disabled {
        opacity: 0.5 !important;
        cursor: not-allowed !important;
        pointer-events: auto !important;
      }
  ...
  async init() {
    // Check if any ef-geo attributes exist on the page
    const geoElements = document.querySelectorAll('[ef-geo]');
    const hasGeoAttributes = geoElements.length > 0;

    // Check if any geo attributes need IP-based geolocation
    const ipKeywords = ['location', 'country-name', 'country-code', 'city', 'region'];
    const needsIpGeo = Array.from(geoElements).some(el =>
      ipKeywords.includes(el.getAttribute('ef-geo'))
    );

    await Promise.all([
      this.loadBlockedDomains(),
      this.loadCountryCodes(),
      needsIpGeo ? this.loadGeoData() : Promise.resolve()
    ]);

    this.setupForms();

    // Apply geo auto-selection AFTER forms are fully set up
    if (hasGeoAttributes) {
      this.applyGeoDefaults();
    }
  }
  ...
  async loadGeoData() {
    try {
      ...
      this.geoData = {
        ...
        city: data.city || '',                         // e.g. "Bengaluru"
        region: data.region || '',                     // e.g. "Karnataka"
        callingCode: data.country_calling_code || ''   // e.g. "+91"
      };

      console.log(`Geo detected: ${this.geoData.countryName} (${this.geoData.countryCode})`);
    } catch (error) {
  ...
  // ============================================================================
  // GEO AUTO-SELECTION (applied after forms are set up)
  // ============================================================================
  //
  // USAGE:
  //   ef-geo="location"       → On Country Code phone select: auto-selects
  //                              country by ISO2 code and triggers phone setup
  //
  //   ef-geo="country-name"   → On any <select>: auto-selects option whose
  //                              text or value matches user's country name
  //
  //   ef-geo="country-code"   → On any <select>: auto-selects option whose
  //                              text or value matches user's ISO2 code (e.g. "IN")
  //
  //   ef-geo="city"           → On any <input>: auto-fills user's city name
  //
  //   ef-geo="region"         → On any <input>: auto-fills user's region/state
  //
  //   If ef-geo is NOT present on an element, nothing happens — fully opt-in.
  // ============================================================================

  applyGeoDefaults() {
    const geoElements = document.querySelectorAll('[ef-geo]');

    // Reserved keywords for IP-based geolocation
    const reservedKeywords = ['location', 'country-name', 'country-code', 'city', 'region'];

    geoElements.forEach(el => {
      const geoType = el.getAttribute('ef-geo');

      switch (geoType) {
        case 'location':
          if (this.geoData) this.applyGeoLocation(el);
          break;

        case 'country-name':
          if (this.geoData) this.applyGeoCountryName(el);
          break;

        case 'country-code':
          if (this.geoData) this.applyGeoCountryCode(el);
          break;

        case 'city':
          if (this.geoData) this.applyGeoTextInput(el, this.geoData.city);
          break;

        case 'region':
          if (this.geoData) this.applyGeoTextInput(el, this.geoData.region);
          break;

        default:
          // Treat any non-keyword value as a direct country/option name
          // e.g. ef-geo="India", ef-geo="United States", ef-geo="Germany"
          this.applyGeoDirectMatch(el, geoType);
          break;
      }
    });
  }

  // --- ef-geo="location" : Auto-select country in Country Code phone select ---
  applyGeoLocation(select) {
    if (select.tagName !== 'SELECT') return;

    const iso2 = this.geoData.countryCode.toUpperCase();

    // Find the matching option by iso2 data attribute
    const options = Array.from(select.options);
    const matchIndex = options.findIndex(opt => {
      return (opt.dataset.iso2 || '').toUpperCase() === iso2;
    });

    if (matchIndex === -1) {
      console.warn(`Geo: No matching country code for ISO2 "${iso2}"`);
      return;
    }

    select.selectedIndex = matchIndex;

    // Trigger the change event so handleCountryCodeChange fires
    // (enables phone input, sets min/max length, placeholder, etc.)
    select.dispatchEvent(new Event('change', { bubbles: true }));

    // Show short text since this is auto-selected (closed state)
    const selected = select.options[matchIndex];
    if (selected && selected.dataset.shortText) {
      selected.textContent = selected.dataset.shortText;
    }

    console.log(`Geo: Auto-selected country code for ${this.geoData.countryName}`);
  }

  // --- ef-geo="country-name" : Auto-select option matching country name ---
  applyGeoCountryName(select) {
    if (select.tagName !== 'SELECT') return;

    const countryName = this.geoData.countryName.toLowerCase();
    const options = Array.from(select.options);

    const matchIndex = options.findIndex(opt => {
      const optText = opt.textContent.trim().toLowerCase();
      const optValue = opt.value.trim().toLowerCase();
      return optText === countryName || optValue === countryName;
    });

    if (matchIndex === -1) {
      console.warn(`Geo: No matching option for country name "${this.geoData.countryName}"`);
      return;
    }
    ...
    console.log(`Geo: Auto-selected "${this.geoData.countryName}" in country name dropdown`);
  }

  // --- ef-geo="country-code" : Auto-select option matching ISO2 code ---
  applyGeoCountryCode(select) {
    if (select.tagName !== 'SELECT') return;

    const code = this.geoData.countryCode.toUpperCase();
    const options = Array.from(select.options);

    const matchIndex = options.findIndex(opt => {
      const optText = opt.textContent.trim().toUpperCase();
      const optValue = opt.value.trim().toUpperCase();
      return optText === code || optValue === code;
    });

    if (matchIndex === -1) {
      console.warn(`Geo: No matching option for country code "${code}"`);
      return;
    }
    ...
    console.log(`Geo: Auto-selected ISO2 "${code}" in country code dropdown`);
  }

  // --- ef-geo="city" / ef-geo="region" : Auto-fill text input ---
  applyGeoTextInput(input, value) {
    if (!value) return;

    if (input.tagName === 'INPUT' || input.tagName === 'TEXTAREA') {
      input.value = value;
      input.dispatchEvent(new Event('input', { bubbles: true }));
      console.log(`Geo: Auto-filled "${value}" into ${input.getAttribute('ef-geo')} field`);
    }
  }

  // --- ef-geo="India" / ef-geo="United States" etc. : Direct match by name ---
  // Works on BOTH <select> and Country Code selects
  // Does NOT require IP geolocation — purely attribute-driven
  applyGeoDirectMatch(el, matchValue) {
    if (!matchValue || matchValue.trim() === '') return;

    const target = matchValue.trim().toLowerCase();

    if (el.tagName === 'SELECT') {
      const options = Array.from(el.options);

      // First try: match by option text content or value (for regular selects)
      let matchIndex = options.findIndex(opt => {
        const optText = opt.textContent.trim().toLowerCase();
        const optValue = opt.value.trim().toLowerCase();
        return optText === target || optValue === target;
      });

      // Second try: partial match — option text contains the value
      // Handles Country Code selects where text is "🇮🇳 India +91"
      if (matchIndex === -1) {
        matchIndex = options.findIndex(opt => {
          const optText = opt.textContent.trim().toLowerCase();
          return optText.includes(target);
        });
      }

      // Third try: match by data-country attribute (Country Code select options)
      if (matchIndex === -1) {
        matchIndex = options.findIndex(opt => {
          return (opt.dataset.country || '').toLowerCase() === target;
        });
      }

      // Fourth try: match by data-iso2 attribute (e.g. ef-geo="IN")
      if (matchIndex === -1) {
        matchIndex = options.findIndex(opt => {
          return (opt.dataset.iso2 || '').toLowerCase() === target;
        });
      }

      if (matchIndex === -1) {
        console.warn(`Geo: No matching option for "${matchValue}"`);
        return;
      }
      ...
      // Show short text if this is a Country Code select (closed state)
      const selectedOpt = el.options[matchIndex];
      if (selectedOpt && selectedOpt.dataset.shortText) {
        selectedOpt.textContent = selectedOpt.dataset.shortText;
      }

      console.log(`Geo: Auto-selected "${matchValue}" via direct match`);

    } else if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
      // For inputs, set the value directly
      el.value = matchValue.trim();
      el.dispatchEvent(new Event('input', { bubbles: true }));
      console.log(`Geo: Auto-filled "${matchValue}" into input via direct match`);
    }
  }
  ...
  setupCountryCodeSelects(formData) {
    ...
    selects.forEach(select => {
      ...
      this.countryCodes.forEach(country => {
        ...
        option.dataset.fullText = `${country.flag} ${country.name} ${country.dial_code}`;
        option.dataset.shortText = `${country.flag} ${country.dial_code}`;
      ...
      if (phoneInput) {
        ...
        // Mark this phone input as having an associated country code select
        phoneInput.dataset.hasCountrySelect = 'true';

        // Disable phone input until country code is selected
        phoneInput.disabled = true;
        phoneInput.classList.add('ef-phone-disabled');

        // Listen for click on disabled phone input to show error
        phoneInput.addEventListener('mousedown', (e) => {
          if (phoneInput.disabled) {
            e.preventDefault();
            this.showDisabledPhoneError(phoneInput, formData);
          }
        });
        ...
        // Swap option text: full text when dropdown open, short text when closed
        this.setupSelectTextSwap(select);
  ...
  // ============================================================================
  // SHOW ERROR ON DISABLED PHONE INPUT CLICK
  // ============================================================================

  showDisabledPhoneError(phoneInput, formData) {
    ...
    const errorMsg = 'Please select country code first';

    // Show error message
    if (inputData.msgElement) {
      inputData.msgElement.style.setProperty('display', 'block', 'important');
      inputData.msgElement.style.setProperty('visibility', 'visible', 'important');
      inputData.msgElement.style.setProperty('opacity', '1', 'important');
      inputData.msgElement.textContent = errorMsg;
    }

    // Show wrong indicator
    if (inputData.wrongIndicator) {
      inputData.wrongIndicator.style.setProperty('display', 'flex', 'important');
    }
    if (inputData.rightIndicator) {
      inputData.rightIndicator.style.setProperty('display', 'none', 'important');
    }

    // Apply invalid styling
    phoneInput.classList.add('ef-invalid');
    phoneInput.classList.remove('ef-valid');

    if (inputData.hasBorderColor) {
      const colors = this.getValidationColors(formData.theme);
      phoneInput.style.setProperty('border-color', colors.borderWrong, 'important');
    }
    if (inputData.hasBgColor) {
      const colors = this.getValidationColors(formData.theme);
      phoneInput.style.setProperty('background-color', colors.bgWrong, 'important');
    }

    // Sync country code select styling
    if (phoneInput.dataset.countrySelectId) {
      const select = document.getElementById(phoneInput.dataset.countrySelectId);
      if (select) {
        select.classList.add('ef-invalid');
        select.classList.remove('ef-valid');
        if (inputData.hasBorderColor) {
          select.style.setProperty('border-color', phoneInput.style.borderColor, 'important');
        }
        if (inputData.hasBgColor) {
          select.style.setProperty('background-color', phoneInput.style.backgroundColor, 'important');
        }
      }
    }
  }
  ...
  // ============================================================================
  // SELECT TEXT SWAP: Full text in dropdown, short text when closed
  // ============================================================================

  setupSelectTextSwap(select) {
    // Show short text on the selected option (closed state)
    const showShortText = () => {
      // First restore all options to full text (for next open)
      Array.from(select.options).forEach(opt => {
        if (opt.dataset.fullText) {
          opt.textContent = opt.dataset.fullText;
        }
      });
      // Then set selected option to short text
    ...
    // Restore full text on ALL options (open state)
    const showFullText = () => {
      Array.from(select.options).forEach(opt => {
        if (opt.dataset.fullText) {
          opt.textContent = opt.dataset.fullText;
    ...
    // When dropdown opens → show full text for browsing
    select.addEventListener('focus', showFullText);
    select.addEventListener('mousedown', showFullText);

    // When dropdown closes → always show short text on selected
    select.addEventListener('blur', () => {
      setTimeout(showShortText, 0);
    });

    // When value changes → swap text
    select.addEventListener('change', () => {
      requestAnimationFrame(showShortText);
    });
  }
  ...
  handleCountryCodeChange(select, phoneInput, formData) {
    ...
    if (selectedOption && selectedOption.dataset.minLength && selectedOption.dataset.maxLength) {
      ...
      // Enable phone input after country code is selected
      phoneInput.disabled = false;
      phoneInput.classList.remove('ef-phone-disabled');

      // Clear the "select country code first" error when country is selected
      const inputData = formData.inputs.get(phoneInput);
      if (inputData) {
        // Hide error message
        if (inputData.msgElement) {
          inputData.msgElement.style.setProperty('display', 'none', 'important');
        }

        // Reset invalid state from disabled click
        phoneInput.classList.remove('ef-invalid');
        if (inputData.hasBorderColor) phoneInput.style.removeProperty('border-color');
        if (inputData.hasBgColor) phoneInput.style.removeProperty('background-color');
        if (inputData.wrongIndicator) inputData.wrongIndicator.style.setProperty('display', 'none', 'important');

        // Reset select styling too
        select.classList.remove('ef-invalid');
        select.style.removeProperty('border-color');
        select.style.removeProperty('background-color');

        // Reset interaction state so blur validation works fresh
      ...
      if (inputData && inputData.hasInteracted && phoneInput.value.trim() !== '') {
  ...
  // ============================================================================
  // DEFAULT ERROR MESSAGE (with phone format fix)
  // ============================================================================
  getDefaultErrorMessage(inputData) {
    ...
    // Phone field: check if it has an associated country code select or is standalone
    if (inputData.type === 'phone') {
      const hasCountrySelect = inputData.element.dataset.hasCountrySelect === 'true';

      if (hasCountrySelect) {
        // Has country code select — show "select country code first" if no country selected
        const minLength = inputData.element.dataset.minLength;
        if (!minLength) {
          return 'Please select country code first';
        }
        // Country selected but field empty — show digits-only format
        placeholder = placeholder.replace(/[^\d]/g, '');
        return `Please enter ${placeholder} format`;
      } else {
        // Standalone phone field (no country code select) — simple message
  ...
  resetForm(formName) {
    ...
    formData.inputs.forEach((inputData) => {
      ...
      if (inputData.type === 'phone') {
        ...
        // Re-disable phone input if it has an associated country code select
        if (inputData.element.dataset.hasCountrySelect === 'true') {
          inputData.element.disabled = true;
          inputData.element.classList.add('ef-phone-disabled');
        }
      }
